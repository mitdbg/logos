...
def waterfall_tag(
    ...
    tag, origin = preceding_tokens_tag(variable_row, banned_values)
    if tag != name:
        return (tag, origin)
    ...
    try:
        tag = gpt_tag(templates_df, variable_row, "gpt-4", banned_values)
        if tag != name:
...
def preceding_tokens_tag(
    ...
    Returns:
        A tuple containing (i) the tag for the parsed variable, and (ii) the origin of the tag.
    """
    check_fields(variable_row, ["Preceding 3 tokens", "Name", "From regex"])
    name = variable_row["Name"]
    if variable_row["From regex"]:
        return name, TagOrigin.REGEX_VARIABLE
...
def gpt_tag(
    ...
    """
    check_fields(variable_row, ["Name", "Examples"])
    check_columns(templates_df, ["TemplateId", "TemplateExample"])
...
def deduplicate_tags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    check_columns(df, ["Name", "Tag", "TagOrigin"])
    seen_tags = set()
