...
from src.logos.interactive_causal_graph_refiner import InteractiveCausalGraphRefinerMethod
import datetime
import os
import argparse
import json
...
def interactive_causal_graph_refinement(dataset: str, methods: list[str]):
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    runlog_path = os.path.join(outdir, f"bounded_{dataset}_runlog.txt")
    ...
    for filename in os.listdir(indir):
        if filename.endswith(conf["suffix"]):
            ...
            gpt_log_path = os.path.join(outdir, f"bounded_{filename[:-4]}_gpt_log.txt")
            ...
            for method in methods:
                ...
                for j in range(JUDGMENTS):
                    edge, latency = s.get_causal_graph_refinement_suggestion(
                        method_obj, conf["treatment"], conf["outcome"], gpt_log_path=gpt_log_path
                    )
                    print(f"Edge: {edge}")
                    ...
                    if edge is not None:
                        if list(edge) in conf["true_graph_edges"]:
                            ...
                            print("The inverse of this edge is in the ground truth graph")
    ...
    f.close()
    for method in methods:
        results_path = os.path.join(outdir, f"{dataset}_bounded_results_{method}.csv")
        latency_path = os.path.join(outdir, f"{dataset}_bounded_latency_{method}.csv")
