...
for dataset in datasets:
    ...
    for method in methods:
        path = os.path.join(
            ...
            f"{dataset}_bounded_results_{method}.csv",
        )
        results[method] = pd.read_csv(path, header=0)
        results[method] = results[method].drop(columns=["edge"])
        results[method] = results[method].groupby("judgments").mean()[["ARE_ATE"]]
    ...
    #####
    # Plot the ARE_ATE trajectory of each method.
    _, ax = plt.subplots(figsize=(6, 5))
    for method in methods:
        ax.plot(
            results[method]["judgments"],
            results[method]["ARE_ATE"],
    ...
    ax.set_ylabel("Mean ARE_ATE", fontsize=FONTSIZE, labelpad=10)
    ...
    fig_path = os.path.join(
        ...
        "evaluation",
        f"{prefix}_plots",
        f"8.3-interactive-causal-graph-refinement-{dataset}-bounded-are-ate-trajectory.png",
    )
    ...
    res_path = os.path.join(
        ...
        "evaluation",
        f"{prefix}_results",
        f"8.3-interactive-causal-graph-refinement-{dataset}-bounded-are-ate-trajectory.csv",
    )
    if not os.path.exists(os.path.dirname(res_path)):
        os.makedirs(os.path.dirname(res_path))
    with open(res_path, "w+") as f:
        f.write("Method,Judgment,ARE_ATE\n")
        for method in methods:
            for i, row in results[method].iterrows():
                f.write(f"{method},{i},{row['ARE_ATE']}\n")
