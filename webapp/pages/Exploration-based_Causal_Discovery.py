...
if 'is_prepared' in st.session_state and st.session_state['is_prepared']:
    st.markdown("You have a collection of actions at your disposal, each of which is given a horizontal section below:")
    st.markdown('<hr style="border:1px solid lightgray">', unsafe_allow_html=True)


    # Row about exploration
    st.subheader("Explore candidate causes")
    col_1, col_2 = st.columns([0.3, 0.7])
    with col_1:
        sawmill_ui.prompt_explore()
    with col_2:
    ...
    col_1, col_2 = st.columns([0.3, 0.7])
    with col_1:
        sawmill_ui.prompt_inspect()
        
    with col_2:
        with st.expander("Inspection results", expanded=True):
            if "base_var_info_df" in st.session_state:  
                st.markdown("Information about this variable's base variable from the log:")
            ...
            if "prepared_log_info_df" in st.session_state:
                st.markdown("Information about this variable's values in the prepaerd table:")
